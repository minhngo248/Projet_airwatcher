        ...
        else if (lecture == "3")
        {
            ...
            while (!(lecture == "2"))
            {
                if (lecture == "1")
                {
                    ...
                    cin >> lecture2;
                    auto start = std::chrono::high_resolution_clock::now();
                    Administrateur AdminConnecte = system.SeConnecterAdmin(lecture1, lecture2);
                    auto finish = std::chrono::high_resolution_clock::now();
                    std::chrono::duration<double> elapsed = finish - start;
                    cout << "Temps d'exécution: " << elapsed.count() << " s\n";

                    if (AdminConnecte.GetId() != -1)
                    {
                        cout << "\n----------------- Menu (2) Admin: --------------" << endl;
                        ...
                        while (!(lecture == "4"))
                        {
                            if (lecture == "1")
                            {
                                ...
                                cin >> lecture6;
                                auto start = std::chrono::high_resolution_clock::now();
                                system.VerifierFiabiliteCapteur(AdminConnecte, lecture5, lecture6);
                                auto finish = std::chrono::high_resolution_clock::now();
                                std::chrono::duration<double> elapsed = finish - start;
                                cout << "Temps d'exécution: " << elapsed.count() << " s\n";
                            }
